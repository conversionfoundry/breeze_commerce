...
module Breeze
  module Commerce
    class ProductList < Breeze::Content::Item
      ...
      attr_accessible :title, :list_type, :product_id, :tag_ids, :container_id, :region, :view, :use_pagination
      ...
      def to_erb(view, page_number=1)
        products_local = use_pagination? ? Kaminari.paginate_array(products).page(page_number).per(1) : products
